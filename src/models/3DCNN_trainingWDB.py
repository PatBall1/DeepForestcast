...
wandb.init(config=hyperparameter_defaults, project="forecasting", entity="patball")
...
if __name__ == "__main__":
    ...
    if not (
        os.path.isfile(wherepath + "/" + "Train3D_idx%d.npy" % (config["end_year"]))
        & os.path.isfile(wherepath + "/" + "Test3D_idx%d.npy" % (config["end_year"]))
    ):
        ...
        np.save(wherepath + "/" + "Train3D_idx%d.npy" % (config["end_year"]), train_idx)
        np.save(wherepath + "/" + "Test3D_idx%d.npy" % (config["end_year"]), test_idx)
    else:
        print(
            "loading: " + wherepath + "/" + "Train3D_idx%d.npy" % (config["end_year"])
        )
        train_idx = np.load(
            wherepath + "/" + "Train3D_idx%d.npy" % (config["end_year"])
        )
